...
$traer = "SELECT * FROM recibo  ORDER BY idRecibo DESC LIMIT 1";
$row1 = mysqli_query($mysqli, $traer);
$mues = mysqli_fetch_assoc($row1);



$sql = "SELECT * FROM recibo INNER JOIN tipogasolina ON tipogasolina.idTGaso = recibo.idTGaso INNER JOIN tipovehiculo ON tipovehiculo.idTvehiculo = recibo.idTvehiculo INNER JOIN usuarios ON usuarios.documento = recibo.idUs where recibo.idRecibo = $mues[idRecibo]";
